...
=======
On the XGene-1, when an incoming frame is received, the controller is capable
...
space of the process but a physical frame is not associated until the first page fault.
In this case, however, the newly allocated address must be communicated straight 
away to the hardware which access system memory without going through the CPU MMU. 
Therefore a mapping must be available since the very beginning. The device is
not always capable of DMAing directly to physical addresses. Usually there is IOMMU
hardware that translates addresses as seen by the device to physical addresses:
...
initialization and is basically responsible for the following operations:
...
The DMAable addresses that are passed to the hardware ring buffer point directly
to the *data* field of the *sk_buff*s. A further hypothesis that I wanted to validate
was whether corruption was happening when data was DMAed and subsequently read from
memory, due to faulty RAM.


Validating frames after DMA transfers
=======
In order to check whether *xgene_enet_rx_frame* was receiving data already corrupted, 
I wrote some code that would perform the following steps:
    
  * Trap *xgene_enet_rx_frame*
  * Calculate the CRC and compare it with the one in the FCS field
  * If a mismatch is detected, then print the physical address of the frame
...
of inspecting the function's arguments. However, here comes a problem: as of the
latest upstream kernel version at the time of writing (4.7.0) jprobes are not
officially supported on ARM64 architecture. However,
several implementations have already been circulated in the kernel mailing list.
the latest one being from Sandeepa Prabhu on the 8th of July 2016
(*arm64: Kprobes with single stepping support*). This serie of 10 patches cleanly
applies to kernel 4.6.0, aka 2dcd0af568b0cf583645c8a317dd12e344b1c72a), so
that's the version I used. Also, in the kernel configuration I had to disable
*CONFIG_ARM64_ERRATUM_843419* to work around a "unsupported RELA" error when
loading the module.
